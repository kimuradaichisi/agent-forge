...
run rm -rf "$SKILL_DIR/lean-routing" "$SKILL_DIR/scientific-debugging" "$SKILL_DIR/spec-driven-dev" "$SKILL_DIR/pr-guardian" "$SKILL_DIR/codebase-cartographer"
case "$PLATFORM" in
  ...
  gemini) for f in cheap-ops.md cheap-edit.md cheap-coder.md reviewer.md; do run rm -f "$AGENTS/$f"; done; run rm -f "$CMDS/lean.toml" "$CMDS/debug.toml" "$CMDS/spec.toml" "$CMDS/pr.toml" "$CMDS/map.toml";;
esac
if [[ -f "$INST" ]]; then
